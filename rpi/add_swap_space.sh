...
# the Raspberry Pi is fairly limited on RAM, especially if we want to build on $
# so we enable a large amount of swap memory, which means using flash memory as$
...
# this search and replace will overwrite whatever swap memory value it was befo$
